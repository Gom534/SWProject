...
class menuFragment : Fragment() {
    ...
    private var restname: String = arguments?.getString("restname")?:""
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        sharedPreferences = requireContext().getSharedPreferences(PREFSNAME, Context.MODE_PRIVATE)
        checkAndResetCounter()
        restname = arguments?.getString("restname")?:""
        Log.d("restname" , restname)
        // 버튼 ID를 사용하여 버튼을 찾습니다.
    ...
    private fun getCategoriesAndAllData() {
        ...
        categoriesRef.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                for ((index, categorySnapshot) in snapshot.child("BHC").children.withIndex()) {
                    ...
                    if (categoryName != null) {
                        ...
                        if (index < childrenCount - 1) {
                            val dottedLine = View(requireContext()).apply {
                                layoutParams = LinearLayout.LayoutParams(
                                    LinearLayout.LayoutParams.MATCH_PARENT,
                ...
                cartbtn = Button(requireActivity()).apply {
                    layoutParams=LinearLayout.LayoutParams(
                        LinearLayout.LayoutParams.MATCH_PARENT,
                        LinearLayout.LayoutParams.WRAP_CONTENT
                    )
                    setOnClickListener{
                        val intent1 = Intent(requireActivity(), Cart::class.java)
                ...
                buttonContainer.addView(cartbtn)
